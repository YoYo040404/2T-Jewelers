  ...
  return (
    <div style={{ display: 'inline-flex', alignItems: 'center', gap: 10 }}>
      ...
      <div style={{ lineHeight: 1 }}>
        ...
        <div className="label" style={{ color: 'var(--fg-mute)', fontSize: 9, marginTop: 4, letterSpacing: '0.24em' }}>CUSTOM · CATALOG · INQUIRY</div>
      </div>
    </div>
  );
}
...
function Hero({ variant = 'mobile' }) {
  ...
  return (
    <section data-screen-label={`${variant} · Hero`} style={{
      position: 'relative',
      padding: isMobile ? '20px 16px 32px' : '64px 56px 72px',
      overflow: 'hidden',
    }}>
      ...
      <div style={{ position: 'relative', zIndex: 3, maxWidth: isMobile ? 'none' : 1100 }}>
        <div className="eyebrow" style={{ marginBottom: isMobile ? 14 : 22 }}>
          Signature Pieces · Custom Builds · Inquiry First
        </div>
        <h1 className="serif" style={{
          margin: 0,
          fontSize: isMobile ? 39 : 88,
          lineHeight: isMobile ? 0.97 : 0.92,
        ...
        <p style={{
          ...
          Cuban links, iced pendants, tennis chains, watches, grillz, and one-of-one customs.
          Send a reference, get clear options, and start the piece with confidence.
        </p>
        ...
        <div style={{
          ...
          {[
            { n: 'Custom', l: 'Built around your reference' },
            { n: '10K / 14K', l: 'Solid gold options' },
            { n: 'VVS', l: 'Stone clarity options' },
            { n: 'Inquiry', l: 'Fast response flow' },
          ].map((t, i) => (
            <div key={i}>
...
function TrustStrip({ variant }) {
  ...
  const items = [
    ...
    { ic: Ic.clock, t: 'Flexible inquiry flow' },
    { ic: Ic.ship, t: 'Delivery options discussed' },
    { ic: Ic.pin, t: 'Showroom details on request' },
