...
function Header({ variant, onMenu }) {
  ...
  return (
    <header style={{
      ...
      padding: isMobile ? '10px 14px' : '12px 40px',
        ...
        <a href="#" style={{
          ...
          <div style={{ padding: isMobile ? '22px 18px' : '44px 40px', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
            ...
            <div style={{ marginTop: 22, display: 'flex', gap: 10, flexWrap:'wrap' }}>
              <span className="chip gold dot">Timeline shared with quote</span>
              <span className="chip">Design details confirmed upfront</span>
...
function ProductCard({ p, variant, cardStyle = 'editorial' }) {
  ...
  return (
    <a href="#" style={{
      ...
      padding: isCommerce ? 0 : (isMobile ? 10 : 12),
      backgroundColor: isCommerce ? 'var(--ink-2)' : 'rgba(255,255,255,0.012)',
    }}
    onMouseOver={e => { e.currentTarget.style.borderColor = 'rgba(212,162,76,0.4)'; }}
    onMouseOut={e => { e.currentTarget.style.borderColor = 'rgba(212,162,76,0.14)'; }}>
      <div className="ph" style={{ aspectRatio: '1 / 1.05', position: 'relative', border: isCommerce ? 'none' : '1px solid rgba(212,162,76,0.15)' }}>
      ...
      <div style={{ padding: isCommerce ? (isMobile ? '14px 14px' : '18px 18px') : (isMobile ? '14px 2px 2px' : '14px 4px 4px') }}>
...
function BestSellers({ variant, cardStyle }) {
  ...
  return (
    <section data-screen-label={`${variant} · Best Sellers`} style={{
      ...
      <div style={{ display: 'grid', gap: isMobile ? 12 : 18, gridTemplateColumns: `repeat(${cols}, 1fr)` }}>
